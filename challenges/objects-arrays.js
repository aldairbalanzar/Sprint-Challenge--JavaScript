...
function getUni (list) {
  let universities = [];
  list.forEach(student => universities.push(`${student.university}`));
...
function getContactInfo(list) {
  let contactInfo = [];
  list.forEach(student => contactInfo.push(`${student.first_name} ${student.email}`));
  return contactInfo;
}
...
let unisWithUni = graduates.map((item) => {
...
function displayNames(list) {
  let displayNames = [];
  list.forEach(animal => displayNames.push(`Name: ${animal.animal_name}, Scientific: ${animal.scientific_name}`));
  return displayNames;
}
...
function lowerCaseNames(list) {
 return list.map(animal => animal.animal_name.toLowerCase());
}
...
*/
function lowPopulation(list) {
  return list.filter(animal => animal.population < 5);
}
